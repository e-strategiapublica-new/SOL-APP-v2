...
@Component({
  selector: 'app-licitacoes-revisao',
  templateUrl: './licitacoes-revisao.component.html',
  styleUrls: ['./licitacoes-revisao.component.scss']
})
export class LicitacoesRevisaoComponent {
  licitacoesList: any = [];
  ...
  list(){
      ...
      if(this.user.roles === 'gerente_geral_projetos') {
        this._associationBidService.getByProjectManagerId(this.userId).subscribe({
          next: (data: any) => {
        ...
        this._associationBidService.getByReviewerId(this.userId).subscribe({
          next: (data: any) => {
            this.licitacoesList = data.filter((item : any) => {
